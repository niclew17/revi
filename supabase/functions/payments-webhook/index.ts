...
// Initialize Stripe with error handling for expired keys
let stripe: Stripe;
try {
  ...
  // Create a mock stripe object to prevent runtime errors
  stripe = null as any;
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (!stripe) {
      console.error(
        "Stripe not initialized - likely due to expired or missing API key",
      );
      return new Response(
        JSON.stringify({ error: "Stripe service unavailable" }),
        {
          status: 503,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
